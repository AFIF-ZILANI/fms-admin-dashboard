...
import type { WastageByCategoryRow } from "@/pages/analytics/types";

type WastageByCategoryChartProps = { days: number };

export function WastageByCategoryChart({ days }: WastageByCategoryChartProps) {
  const { data, isLoading, isError } = useGetData<WastageByCategoryRow[]>(
    `/analytics/inventory/wastage-by-category?days=${days}`,
    ["analytics", "inventory", "wastage-by-category", days]
  );
  ...
  return (
    <Card>
      <CardHeader>
        <CardTitle className="text-base">Wastage & expiry losses</CardTitle>
      </CardHeader>
      <CardContent>
        ...
        {!isLoading && !isError && rows.length === 0 && (
          <EmptyState
            icon={TriangleAlert}
            title="No wastage or expiry losses logged"
