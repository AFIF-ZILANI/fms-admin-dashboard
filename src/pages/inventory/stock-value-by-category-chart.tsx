...
import type { StockValueByCategoryRow } from "@/pages/analytics/types";

/** Current on-hand value -- a snapshot, not a trend, so no day range. */
export function StockValueByCategoryChart() {
  const { data, isLoading, isError } = useGetData<StockValueByCategoryRow[]>(
    "/analytics/inventory/stock-value-by-category",
    ["analytics", "inventory", "stock-value-by-category"]
  );
    ...
    <Card>
      ...
      <CardContent>
        ...
        {!isLoading && !isError && rows.length === 0 && (
          <EmptyState
            icon={BarChart3}
            title="No valued stock on hand"
            description="Items need purchase history to compute a value."
          />
        )}
        {!isLoading && !isError && rows.length > 0 && (
